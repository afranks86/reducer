...
The steps in the algorithm which yield a valid solution are:
...
2. Identify the null space of $\beta$ outside the nullspace of $\alpha$: $N_{\beta/\alpha}$
    + select a random set of coordinate directions $w$ so that $ww^T$ is a projection $N_{\beta/\alpha}$
...
I think this yields an exhaustive set of solutions but haven't figured out a good way to  parameterize to control the correlation of $d(X)$ with $\alpha$ and $\beta$.
...
# p/2 is arbitrary but should balance alpha/beta contribution 
...
print(max(abs(cor(m, d))))
print(max(abs(cor(e, d))))
print(cor(m, e))
...
## Notes on envelope model

- $X = BT + eps$
- partial envelope?
- heteroscedastic covariance $V\psi_tV^T$