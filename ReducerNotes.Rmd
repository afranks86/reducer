...
where $U\Lambda U'$ is the eigendecomposition of M.  Specifically, U is  $p \times 2$ matrix of eigenvectors for the 2 non-zero eigenvalues (first is positive eval and second is negative eval).

Then we have a solution characterized by the hyperbola

$$ \frac{\lambda_1}{\alpha'\beta}w_1 - \frac{\lambda_2}{\alpha'\beta}w_2 = 1$$
for $w_i = \gamma'u_i$ and $\lambda_2$ is the absolute value of the negative eigenvalue in $\Lambda$.  A couple things to note:
...
alpha <- c(1, -3, 0, rep(0, p-3))
beta <- c(1, 0, .3, rep(0, p-3))
