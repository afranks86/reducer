...
The goal of the reducer is to take an outcome model $m_0(x)$ and a treatment model $e(X)$ and to identify a function conditional on which the two are orthogonal.

With unconfoundedness in $X$, we have
$$
\begin{align}
Cov(Y(0), T \mid s(X)) &= E[Cov(Y(0), T \mid X) \mid s(X)] + Cov(E[Y(0) \mid X], E[T \mid X] \mid  s(X))\\
&= Cov(m_0(X), e(X) \mid s(X)).
\end{align}
$$

The condition for a reducer $s(X)$ is:
$$
Cov(m_0(X), e(X) \mid s(X)) = 0 \quad \forall X
$$

or

$$
MI(m_0(X), e(X) \mid s(X)) = E\left[ \log\left(\frac{p(m_0(X), e(X))}{p(m_0(X))p(e(X))}\right) \mid s(X)\right] = 0 \quad \forall X
$$
