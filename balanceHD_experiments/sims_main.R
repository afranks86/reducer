## Run this from `reducer` root folder (`..` from here)
rm(list = ls())
...
if(!interactive()){
  ...
  beta.setup = 1#as.numeric(args[1])
  prop.setup = 3#as.numeric(args[2])
  n = 500#as.numeric(args[3])
  p = 50#as.numeric(args[4])
  eps = 0.05#as.numeric(args[5])
  C = 2#as.numeric(args[6])
  extra.param = 1#as.numeric(args[7])
...
source("balanceHD_experiments/run.comparison.R")
...
reduce_and_run <- function(XYW, angle=pi/4, include_m=FALSE, type='ATT'){
  if(type=='ATT'){
    reduced = one_sided(XYW$Y, XYW$X, XYW$W, 1, angle, include_m)
  } else {
    reduced = two_sided(XYW$Y, XYW$X, XYW$W, angle, include_m)
  }
	XYW2 = XYW
	XYW2$X = cbind(0, reduced)
	suppressWarnings(run.comparison(XYW2))
}
...
if(!interactive()){
 ...
 # })
  ...
  results = replicate(NREP, {
    XYW = gen.data()
    tryCatch({
    	res1 = suppressWarnings(run.comparison(XYW))
      source('reducer.R')
    	res2 = reduce_and_run(XYW, type='ATE')
    	res3 = reduce_and_run(XYW, include_m=TRUE, type='ATE')
    	res4 = reduce_and_run(XYW, angle=pi/2, include_m=TRUE, type='ATE')
    	cbind(CATT=XYW$catt, cbind(plain=res1, d=res2, dplusm=res3, eplusm=res4))
    }, error=function(e){print(e); NULL})
  })
  
  CATTS = results[,1,]
  res_small = results[,-1,]
  bias_res = sapply(1:dim(res_small)[3],
                    function(l){((res_small[,,l] - CATTS[,l])/CATTS[,l])})
  mse_res = sapply(1:dim(res_small)[3],
                   function(l){((res_small[,,l] - CATTS[,l])/CATTS[,l])^2})
  mse_res = array(mse_res, dim(res_small)[-3])
  dimnames(mse_res) = dimnames(res_small)[1:2]
  print(mse_res)
